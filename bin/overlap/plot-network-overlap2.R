...
dat2 = read.delim("data/networks/overlap.txt.gz") %>%
  ...
  mutate(coefficient=paste0(coefficient, ".original"), type="original")
dat = rbind(dat, dat2)
...
p1 = ggplot(dat1, aes(x = reorder(coefficient, z_score, median), 
  ...
  scale_color_manual(name='', values=c("grey10","lightgray"), guide=F) +
  coord_flip() + 
  clean_theme + 
...
p1
ggsave("results/fig/overlap_1_recomputed+original.pdf", p1, width = 18.3, height = 9, units = 'cm')
...
ggsave("results/fig/overlap_2_recomputed+original.pdf", p4, width = 18.3, height = 18, units = 'cm')
...
for (db in unique(dat$network)){
  ...
  write.table(pvals, paste0("results/overlap/pvals_", db, "_recomputed+original.txt"), quote = F, row.names = T,
              sep = "\t")
