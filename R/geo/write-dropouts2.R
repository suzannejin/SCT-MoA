...
files_geo = list.files("data/geo/filtered", pattern = "*.gz$", full.names = T)
files_10x = list.files("data/10xgenomics.com/filtered", pattern = "*.gz$", 
                       full.names = T)
...
# write
write.csv(df, "data/geo/dropouts2.csv", row.names = F)
